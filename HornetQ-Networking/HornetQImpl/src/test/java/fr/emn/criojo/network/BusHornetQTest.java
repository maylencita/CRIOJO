...
public class BusHornetQTest {
	...
	@Test
	public void cloudRemoteTest() throws FileNotFoundException, IOException,
	    RequiredPropertiesException {
		// Load properties
		final String[] REQUIRED_PROPS = { "cloudRemoteTest.busNodeHost.1",
		    "cloudRemoteTest.busNodePort.1", "cloudRemoteTest.connectorName.1",
		    "cloudRemoteTest.connectorLogin.1",
		    "cloudRemoteTest.connectorPassword.1", "cloudRemoteTest.busNodeHost.2",
		    "cloudRemoteTest.busNodePort.2", "cloudRemoteTest.connectorName.2",
		    "cloudRemoteTest.connectorLogin.2",
		    "cloudRemoteTest.connectorPassword.2" };
		Properties testProps = loadTestProperties(REQUIRED_PROPS);

		// First connector arguments.
		final String BUS_NODE_HOST_1 = testProps.getProperty(REQUIRED_PROPS[0]);
		final int BUS_NODE_PORT_1 = Integer.parseInt(testProps
		    .getProperty(REQUIRED_PROPS[1]));
		final String CONNECTOR_NAME_1 = testProps.getProperty(REQUIRED_PROPS[2]);
		final String CONNECTOR_LOGIN_1 = testProps.getProperty(REQUIRED_PROPS[3]);
		final String CONNECTOR_PASSWORD_1 = testProps
		    .getProperty(REQUIRED_PROPS[4]);

		// Second connector arguments.
		final String BUS_NODE_HOST_2 = testProps.getProperty(REQUIRED_PROPS[5]);
		final int BUS_NODE_PORT_2 = Integer.parseInt(testProps
		    .getProperty(REQUIRED_PROPS[6]));
		final String CONNECTOR_NAME_2 = testProps.getProperty(REQUIRED_PROPS[7]);
		final String CONNECTOR_LOGIN_2 = testProps.getProperty(REQUIRED_PROPS[8]);
		final String CONNECTOR_PASSWORD_2 = testProps
		    .getProperty(REQUIRED_PROPS[9]);
		...
		class Connector1 extends Thread {
			...
			public void run() {
				try {
					conn = new BusConnectorRemoteHornetQ(BUS_NODE_HOST_1,
					    BUS_NODE_PORT_1, CONNECTOR_NAME_1, CONNECTOR_LOGIN_1,
					    CONNECTOR_PASSWORD_1);
		...
		class Connector2 extends Thread {
			...
			public void run() {
				try {
					conn = new BusConnectorRemoteHornetQ(BUS_NODE_HOST_2,
					    BUS_NODE_PORT_2, CONNECTOR_NAME_2, CONNECTOR_LOGIN_2,
					    CONNECTOR_PASSWORD_2);
