...
final case class ArrowAssocCriojoTuple2[T1 <: Pattern with Expression,
  ...
  override def matches(expr: Expression): Boolean = expr match {
    case wst: WrapScalaTuple2[T1, T2] => matchesOfWrapScalaTuple2(wst)
    case _ => false
  }
  ...
  override def getValuation(expr: Expression): Valuation = expr match {
    case wst: WrapScalaTuple2[T1, T2] => getValuationOfWrapScalaTuple2(wst)
    case _ => Valuation()
  }
  ...
  private def matchesOfWrapScalaTuple2(wst: WrapScalaTuple2[T1, T2]):
      Boolean = {
  ...
  private def getValuationOfWrapScalaTuple2(wst: WrapScalaTuple2[T1, T2]):
      Valuation = {
