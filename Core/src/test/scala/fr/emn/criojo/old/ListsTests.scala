...
import fr.emn.criojo.ext.ListCham
...
class ListsTests {
  ...
  val lstCham = new ListCham() with NumCham {
    ...
    val Prnt = NativeRelation("Print") {
      (a, s) => print(a.vars(0).name.span(_ != '@')._1)
    }
    private val PrintItem = NativeRelation("PrintItem") {
      (a, s) =>
        print(a.vars(0) + "::")
    }
    private val PrintVar = NativeRelation("PrintVar") {
      (a, s) =>
        print(a.vars(0))
    }
    private val PrintNil = NativeRelation("PrintNil") {
      (a, s) => print("[]")
    }
...
trait NumCham extends Cham {
  ...
  private val PrintVal = NativeRelation("PV") {
    (a, s) =>
      if (a.vars.isEmpty) {
        print(counter)
        counter = 0
  ...
  private val CreateNum = NativeRelation("CN") {
    (a, s) =>
  ...
  when(PrintNum(x) & Zero(x)) {
    PrintVal() & Zero(x)
  }
  when(PrintNum(x) & Suc(y, x)) {
    Nu(t)(PrintVal(t) & PrintNum(y) & Suc(y, x))
  }
