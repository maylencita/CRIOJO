...
axiom	:	constraint -> axiom(cons= {$constraint.st})
	;			
...
solvparams
	...
	|	^(PARAMS (ns+=newSolv)*) -> paramList(parList={$ns})
	;
...
solvinstance
	:	^(NEW ^(SOLVER ID solvparams?)) -> solvinstance(solvName={$ID.text}, params={$solvparams.st})
	;
