 ...
 * valuation := Top
 *            | Bottom
 *            | (x:Variable -> v:Expression) valuation
 */
trait Valuation {
  ...
  /**Return the expression associated with the given key
   *
   * @return a expression.
   */
  def apply(key: Variable): Option[Expression]

  /**Return the assignment associated with the given key
   *
   * @return an assignment.
   */
  def get(key: Variable): Option[Assignment]
...
class EmptyVal extends Valuation {
  ...
  override def apply(key: Variable) = None

  def get(key: Variable) = None
...
class MapValuation(kv: Set[Assignment], val sign: Boolean = true) extends Valuation {
  ...
  def get(x: Variable) = keyValues.find(asg => asg(x))
   ...
   * @return
   */
  def apply(key: Variable) = keyValues.find(a => a(key)) match {
    case Some(assg) => Some(assg.value)
    case _ => None
  }
...
class ValuationList(protected val vlist: List[NormalForm]) {
  ...
  override def toString = vlist.mkString("[","","]")
}
